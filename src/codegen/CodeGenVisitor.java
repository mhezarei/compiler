...
public class CodeGenVisitor implements SimpleVisitor {
    ...
    private void visitBreakNode(ASTNode node) throws Exception {
        ASTNode block = node.getParent().getParent();
        System.out.println("in break node " + block.getNodeType());

        switch (block.getNodeType()) {
            case METHOD_DECLARATION:
                System.out.println("++++++++++++++++++++++SAW THIS ONE++++++++++++++++++");
                throw new Exception("no break in method_dcl");
            case CASE_STATEMENT:
            case IF_STATEMENT:
            case FOR_STATEMENT:
            case REPEAT_STATEMENT:
                if (breakLabels.peek() != null) {
                    stream.println("\tbr label %" + breakLabels.peek());
                } else {
                    throw new Exception("not in any block");
                }
        }
    }
    ...
    private void visitContinueNode(ASTNode node) throws Exception {
        ...
        switch (block.getNodeType()) {
            case METHOD_DECLARATION:
                throw new Exception("no break in method dcl");
            case CASE_STATEMENT:
            case IF_STATEMENT:
            case FOR_STATEMENT:
            case REPEAT_STATEMENT:
                if (continueLabels.peek() != null) {
                    stream.println("\tbr label %" + continueLabels.peek());
                } else {
                    throw new Exception("not in any block");
                }
        }
    ...
    private void visitSwitchNode(ASTNode node) throws Exception {
        ...
        String[] labels = new String[node.getChild(1).getChildren().size() + 1];

        node.getChild(0).accept(this);

        String result = ((IdentifierNode) node.getChild(0).getChild(0).getChild(0)).getValue();

        Type type = (node.getChild(0).getChild(0).getChild(0)).getSymbolInfo().getType();
        ...
        for (ASTNode block : node.getChild(1).getChildren()) {
            ...
            breakLabels.push(labels[i + 1]);
            continueLabels.push(labels[i + 1]);
    ...
    private void visitVariableDeclaration(ASTNode node) throws Exception {
        ...
        boolean isAuto = typePrimitive == PrimitiveType.AUTO;
        boolean isArray = false;
        if (node.getChild(1).getNodeType() == NodeType.ARRAY_DECLARATION && node.getNodeType() == NodeType.VARIABLE_DECLARATION) {
            if (isAuto)
                throw new Exception("auto not allowed here");
            PrimitiveType type = ((TypeNode) node.getChild(0)).getType().getPrimitive();
            int num = ((IntegerLiteralNode) node.getChild(1).getChild(1)).getValue();
            typePrimitive = new ArrayType(type, num);
        ...
        for (int i = 1; i < node.getChildren().size(); i++) {
            ...
            if (isArray)
                idNode = (IdentifierNode) node.getChild(0);
            else if (node.getChild(i) instanceof IdentifierNode) {
                //DEC -> ID
        ...
        //it is not argument
        if (node.getNodeType() == NodeType.VARIABLE_DECLARATION || isArray) {
            for (int i = 1; i < node.getChildren().size(); i++) {
                IdentifierNode id;
                if (isArray)
                    id = (IdentifierNode) node.getChild(0);
                else if ((node.getChild(i) instanceof IdentifierNode))
                    id = (IdentifierNode) node.getChild(i);
                else
                    id = (IdentifierNode) node.getChild(i).getChild(0).getChild(0).getChild(0);

                stream.println("\t%" + id.getValue() + " = alloca " + typePrimitive + ", align " + typePrimitive.getAlign());

                if (!(node.getChild(i) instanceof IdentifierNode)&&!isArray) {
                    ExpressionNode rightSide = (ExpressionNode) node.getChild(i).getChild(1);
                    ExpressionNode cast = cast(id.getSymbolInfo().getType().getPrimitive(), rightSide);
                    stream.println("\tstore " + cast.getType() + " " + cast.getResultName() + ", " + cast.getType() + "* " + id + ", align " + typePrimitive.getAlign());
                }
            }
        }
    }
    ...
    private void reduceExpressionNode(String result, ExpressionNode parent, Type resultType) throws Exception {
...
//        System.out.println("second child is " + e2.getChildren());
        ...
        Type resultType = checkBinaryOpType(e1.getType().getPrimitive(), e2.getType().getPrimitive(), node.getNodeType());
        // operands are casted now
        String op = getBinaryOperationCommand(e1.getType(), e2.getType(), node.getNodeType(), resultType.getPrimitive());

        if (op.startsWith("f")) {
            if (!(e1.getType() == PrimitiveType.FLOAT || e1.getType() == PrimitiveType.DOUBLE)) {
                e1 = cast(e2.getType().getPrimitive(), e1);
                e1.accept(this);
            } else if (!(e2.getType() == PrimitiveType.FLOAT || e2.getType() == PrimitiveType.DOUBLE)) {
                e2 = cast(e1.getType().getPrimitive(), e2);
                e2.accept(this);
            }
        }
        ...
        if (op.contains("cmp")) {
            if (e1.getType() != e2.getType()) {
                if (e1.getType() == PrimitiveType.LONG || e2.getType() == PrimitiveType.LONG) {
                    ...
                    switch ((PrimitiveType) e1.getType()) {
                        case INT:
                            switch ((PrimitiveType) e2.getType()) {
                                case CHAR:
                                case BOOL:
    ...
    private String getBinaryOperationCommand(Type e1Type, Type e2Type, NodeType nodeType, PrimitiveType resultType) throws Exception {
        String result = "";
    ...
    ExpressionNode cast(PrimitiveType resultType, ExpressionNode e2) throws Exception {
        ...
        PrimitiveType type2 = (PrimitiveType) e2.getType();
        String op = null;
    ...
    private void visitSizeOfNode(ASTNode node) throws Exception {
        ExpressionNode parent = (ExpressionNode) node.getParent();
        Type type = ((TypeNode) node.getChild(0)).getType();
    ...
    private void visitIfStatementNode(ASTNode node) throws Exception {
        ...
        breakLabels.push(ifFalseLabel);
        continueLabels.push(ifFalseLabel);
    ...
    private void visitForNode(ASTNode node) throws Exception {
        ...
        String EALabel = generateLabel();
        String outLabel = generateLabel();
        breakLabels.push(outLabel);
        continueLabels.push(EALabel);
    ...
    private void visitReturnStatementNode(ASTNode node) throws Exception {
        ...
        Type returnType = ((TypeNode) n.getChild(0)).getType();
        ...
        if (node.getChildren().isEmpty()) {
            ...
            try {
                cast(returnType.getPrimitive(), returnExpr);
